...
STATE_PATH = ROOT / "state_kalman_pairs_1min.json"   # KÜLÖN state a 5-min variánstól
LOG_DIR = ROOT / "logs_live"
LOG_DIR.mkdir(exist_ok=True)

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(name)s] %(message)s")
logger = logging.getLogger("kalman_pairs_1min")
...
# ── Kalman + strategy params — 1-min variáns (WF validated 2026-07-29)
# Backtest 68 nap: +$224/hó IS. Walk-forward OOS 13 ablak: +$210/hó, 92% pos, worst -$15.
# K_ENTRY=2.0 (K=2.5 helyett) — 1-min TF-en több signal, jó minőség
# K_EXIT=0.5 (0.3 helyett) — nagyobb noise tolerancia, gyorsabb exit
# R=16 auto-cal az OLS residual variance-ből (5-min 25 volt)
# Q arányos R-rel, 1000× lassabb béta változás mint alpha (érték-drift < ár-drift)
KALMAN_Q_ALPHA = 1.6e-5     # R * 1e-6
KALMAN_Q_BETA  = 1.6e-8     # R * 1e-9
KALMAN_R       = 16.0
K_ENTRY        = 2.0
K_EXIT         = 0.5
...
# Warmup: 1-min bar-okon, 500 elég (nem 2000 mint 5-min-en)
WARMUP_BARS = 500

# Bar TF a resample-hez
BAR_TF = "1min"
...
async def main():
    ...
    log_file = LOG_DIR / f"kalman_pairs_1min_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.log"
    (LOG_DIR / "_kalman_pairs_1min_current.log").write_text(str(log_file))
    logger.info("======================================================================")
    logger.info("MODE: DRY-RUN | KALMAN GOLD-SILVER 1-MIN | log: %s", log_file.name)
    logger.info("======================================================================")
